...
sys.path.append("/media/razor30/Large/PycharmProjects/future_learning")
...
def create_softmax_loss(logits, masks, n_classes, weights=1.):
    """ create softmax loss function """
    masks_hot = tf.one_hot(masks, n_classes)
    total_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
        labels=masks_hot, logits=logits, name='softmax_xentropy')
    normalized_loss = tf.reduce_mean(
        total_loss)
    add_loss(normalized_loss * weights)
    return normalized_loss * weights
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            kwargs = {"mask": label}
            image, label = image_preprocessing_fn(image, train_image_size, train_image_size, **kwargs)
        ...
        def clone_fn(batch_queue):
            ...
            if 'AuxLogits' in end_points:
                slim.losses.softmax_cross_entropy(
                    ...
                    scope='aux_loss')
            create_softmax_loss(logits, labels, dataset.num_classes - FLAGS.labels_offset, weights=1.)
        ...
        out_mask = tf.expand_dims(tf.argmax(logits, axis=-1) * 255 / (dataset.num_classes - FLAGS.labels_offset),
                                  axis=-1)
